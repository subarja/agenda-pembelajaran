        ...
        return [
            ...
            'current_academic_year' => $this->whenLoaded('currentAcademicYear', fn () => $this->currentAcademicYear ? [
                ...
                'tulis_diizinkan' => $this->currentAcademicYear->aktif
                    || ArchiveWriteSetting::instance()->izinkan_tulis,
            ] : null),
            ...
            // aktif (menghindari query sia-sia ketika PKL mati). `is_pembimbing` = guru
            // pembimbing PKL (punya penugasan penempatan).
            //
            // `loadMissing('teacher')` WAJIB: UserResource dipakai untuk SATU user
            // (auth/me, login, profil) yang sering TIDAK meng-eager-load teacher, sehingga
            // guard `relationLoaded` lama membuat is_pembimbing selalu false → menu PKL
            // tak pernah muncul walau mode ON (bug 2026-07-21). loadMissing aman (no-op
            // bila sudah diload) & tak memicu N+1 karena bukan konteks daftar.
            'pkl' => [
                'mode_aktif' => PklMode::isActive(),
                'is_pembimbing' => PklMode::isActive()
                    && PklMode::canFillAgenda($this->resource->loadMissing('teacher')),
            ],
