...
const {
  newDeck, base, foot, sTitle, sHead, cards, sBig, sImage, sSesi, sQuote, sTayang,
  HIJAU, HIJAU2, KUNING, KUNING2, BIRU, BIRU2, KORAL, KORAL2, ORANYE, ORANYE2, UNGU, UNGU2,
  TEKS, TEKS2, PUTIH, BG, F, FH, fs,
} = T;
...
const HARI = [
  {
    ...
    fokus: 'KENALI DIRI', accent: T.HIJAU, tint: T.HIJAU2,
    ...
    sesi: [
      ...
      { jam: '11.00–11.45', judul: 'SESI 3 — Zona, PERAN MURID & Norma Tim',
        ...
        langkah: [
          ...
          ["10'", 'BACAKAN 5 ATURAN JELAJAH. Murid mengulanginya keras-keras. Pasang PAPAN KARTU di dinding. Tim Piket Data menjelaskan: ambil kartu = berangkat, kembalikan = pulang. Slot kosong = tim masih di luar.'],
        ...
        bahan: '5 poster Zona A3 · Papan Kartu (6 slot + 6 kartu tim) · kertas plano · spidol besar · lakban kertas.',
...

  {
    ...
    fokus: 'CARI FAKTA', accent: T.KUNING, tint: T.KUNING2,
    ...
    sesi: [
      ...
      { jam: '07.45–09.15', judul: 'SESI 1 — TURUN LAPANGAN (SISTEM POS)',
        ...
        langkah: [
          ["10'", 'SIAPKAN POS. Anda MENETAP di Pos sepanjang sesi. Tim Piket Data memasang Papan Kartu. Tim K3 memeriksa APD tiap tim (sarung tangan & masker untuk area kotor).'],
          ["5'", 'TIM AMBIL KARTUNYA dari Papan Kartu. Tim Piket mencatat. ANDA TIDAK MENANDATANGANI APA PUN. Bacakan ulang Aturan Jelajah no. 1 & 5.'],
        ...
        bahan: 'LK-2 · Papan Kartu (6 slot + 6 kartu tim) · HP · meteran · stopwatch · sarung tangan & masker · KOTAK P3K · nomor darurat.',
...

  {
    ...
    fokus: 'RASAKAN & PILIH', accent: T.KORAL, tint: T.KORAL2,
    ...
    sesi: [
      ...
      { jam: '07.45–09.15', judul: 'SESI 1 — TURUN LAPANGAN: Wawancara Empati',
        ...
        langkah: [
          ...
          ["55'", 'SISTEM POS BERLAKU LAGI. Anda MENETAP di Pos. Tim ambil kartu dari papan ulang (tujuan berbeda). Tim mewawancarai min. 3 orang: Caraka · Satpam · Ibu/Bapak Kantin · murid kelas X · petugas MBG · OSIS. WAJIB membawa 3 KUTIPAN LANGSUNG (kalimat PERSIS, dalam tanda kutip) + nama & peran. Lapor balik tiap 20 menit.'],
...

  {
    ...
    fokus: 'RANCANG & BUAT', accent: T.BIRU, tint: T.BIRU2,
...

  {
    ...
    fokus: 'BERTINDAK & BERBAGI', accent: T.ORANYE, tint: T.ORANYE2,
    ...
    sesi: [
      ...
      { jam: '07.45–09.15', judul: 'SESI 1 — AKSI NYATA (sistem POS)',
        ...
        langkah: [
          ["10'", 'Tim ambil kartu. Tim K3 memeriksa alat & APD. Tim Dokumentasi mengambil FOTO “BEFORE” — WAJIB, dari sudut yang akan diulang. Tanpa foto before, tidak ada bukti perubahan.'],
        ...
        bahan: 'Karya jadi · alat tangan (obeng, gunting, lakban, cable ties, double tape) · IZIN TITIK AKSI dari Wakasek Sarpras (SUDAH ADA sejak H-3) · HP · Papan Kartu.',
...
HARI.forEach(H => {
  const p = newDeck();
  const A = H.accent, TT = H.tint;

  sTitle(p, {
    kicker: `HARI ${H.n}  ·  ${H.nilai}`,
    title: H.fokus,
    sub: `${H.hari}\n${H.aspek}\n\n${H.hasil}`,
    accent: A, tint: TT,
  }).addNotes('Slide ini untuk guru. Jangan dijadikan pengantar ceramah ke murid.');

  // --- Ringkasan hari ---
  let s = sHead(p, `Hari ${H.n} — ${H.fokus}`, `${H.nilai}  ·  ${H.aspek}`, A);
  s.addShape(p.ShapeType.roundRect, { x: 0.55, y: 1.7, w: 12.25, h: 1.15, fill: { color: TT }, line: { color: A, width: 1.5 }, rectRadius: 0.08 });
  s.addText([{ text: 'HASIL AKHIR HARI INI:  ', options: { bold: true, color: A } }, { text: H.hasil, options: { color: TEKS2 } }],
    { x: 0.9, y: 1.75, w: 11.6, h: 1.05, fontFace: F, fontSize: 21, margin: 0, valign: 'middle', lineSpacing: 27 });

  s.addShape(p.ShapeType.roundRect, { x: 0.55, y: 3.0, w: 12.25, h: 0.85, fill: { color: KUNING2 }, line: { color: KUNING, width: 1.2 }, rectRadius: 0.06 });
  s.addText([{ text: '16 KELAS PARALEL:  ', options: { bold: true, color: 'A0700B' } }, { text: 'cek Kartu Sektor & GELOMBANG kelas Anda. Kelas Anda TIDAK turun lapangan bersamaan dengan 15 kelas lain.', options: { color: '7A5A15' } }],
    { x: 0.9, y: 3.05, w: 11.6, h: 0.75, fontFace: F, fontSize: 19, margin: 0, valign: 'middle', lineSpacing: 24 });

  let ty = 4.0;
  if (H.catatan) {
    s.addShape(p.ShapeType.roundRect, { x: 0.55, y: 4.0, w: 12.25, h: 0.9, fill: { color: KORAL2 }, line: { color: KORAL, width: 1.2 }, rectRadius: 0.06 });
    s.addText([{ text: 'PERHATIKAN WAKTU:  ', options: { bold: true, color: 'C4453B' } }, { text: H.catatan, options: { color: '9E4A45' } }],
      { x: 0.9, y: 4.05, w: 11.6, h: 0.8, fontFace: F, fontSize: 19, margin: 0, valign: 'middle', lineSpacing: 24 });
    ty = 5.05;
  }
  s.addText('SUSUNAN SESI HARI INI', { x: 0.55, y: ty, w: 12.25, h: 0.35, fontFace: F, fontSize: 16, color: A, bold: true, charSpacing: 1.2, margin: 0 });
  H.sesi.forEach((se, i) => {
    const yy = ty + 0.42 + i * 0.45;
    s.addShape(p.ShapeType.roundRect, { x: 0.55, y: yy, w: 2.6, h: 0.38, fill: { color: TT }, line: { color: A, width: 1 }, rectRadius: 0.05 });
    s.addText(se.jam.replace(/\n/g, ' '), { x: 0.55, y: yy, w: 2.6, h: 0.38, fontFace: F, fontSize: 13, color: A, bold: true, align: 'center', valign: 'middle', margin: 0 });
    s.addText(se.judul, { x: 3.3, y: yy, w: 9.5, h: 0.38, fontFace: F, fontSize: 17, color: TEKS, valign: 'middle', margin: 0 });
  });
  foot(p, s);

  // --- Slide [TAYANG] untuk murid ---
  sTayang(p, { judul: H.tayang.judul, isi: H.tayang.isi, img: H.tayang.img, accent: A, tint: TT });

  // --- Sesi detail (auto-split, font 21pt) ---
  H.sesi.forEach(se => sSesi(p, { ...se, accent: A, tint: TT }));

  // --- Penutup ---
  sQuote(p, { text: H.penutup.text, sub: H.penutup.sub, accent: A, tint: TT });
